...
BEGIN
  ...
  IF _package_id IS NOT NULL THEN
    ...
    -- A) Lançamento valor_total
    IF _pkg_lancamento = 'valor_total' THEN
      IF _pkg_type = 'mensal' THEN
        -- Mensalidade: lança o valor cheio em todo mês com pelo menos uma evolução
        -- billable (presente, reposição, falta_remunerada, feriado_remunerado).
        SELECT EXISTS (
          SELECT 1 FROM evolutions
          WHERE patient_id = _patient_id
            AND EXTRACT(MONTH FROM date) = _month
            AND EXTRACT(YEAR FROM date) = _year
            AND attendance_status IN ('presente','reposicao','falta_remunerada','feriado_remunerado')
        ) INTO _has_billable_in_month;
        IF _has_billable_in_month THEN
          _revenue := _revenue + _pkg_valor_total;
        END IF;
      ELSE
        -- Pacotes 'por_sessao' / 'personalizado' (ciclo único, valor fechado):
        -- lança 1× no mês de contratação.
    ...
    IF _pkg_lancamento = 'valor_procedimento' THEN
      ...
      -- Faltas cobradas por grupo seguem fluxo padrão abaixo? não — pacote substitui.
      RETURN _revenue + _services_revenue;
    END IF;
  END IF;

  -- ============ FLUXO SEM PACOTE (legacy: payment_value/payment_type direto) ============
  FOR _evo IN
  ...
  LOOP
    IF _evo.group_id IS NOT NULL THEN
      ...
      IF _evo.attendance_status IN ('presente','reposicao','falta_remunerada','feriado_remunerado') THEN
        _revenue := _revenue + _group_price;
      END IF;
    ELSE
      IF _evo.attendance_status IN ('presente','reposicao','falta_remunerada','feriado_remunerado') THEN
        _revenue := _revenue + _payment_value;
      ELSIF _evo.attendance_status = 'falta' AND _evo.confirmed_attendance IS TRUE THEN
        _revenue := _revenue + _payment_value;
      END IF;
    END IF;
  END LOOP;
