  ...
  try {
    ...
    const [{ data: patient }, { data: avals }, { data: pdis }, { data: evos }] =
      await Promise.all([
        ...
        supabaseAdmin.from("psico_avaliacoes").select("*").eq("patient_id", patient_id)
          .order("data_avaliacao", { ascending: false }).limit(3),
        supabaseAdmin.from("psico_pdi").select("*").eq("patient_id", patient_id)
    ...
    const system = `Você é uma(o) psicopedagoga(o) experiente, redigindo um ${TIPO_LABEL[tipo]} com clareza, ética e linguagem acessível.
REGRAS:
