...
export function RegistrosPanel({ patientId }: Props) {
  ...
  const load = useCallback(async () => {
    const { data, error } = await supabase
      .from('psico_registros').select('*').eq('patient_id', patientId)
  ...
  async function save() {
    ...
    try {
      ...
      const { error } = await supabase.from('psico_registros').insert({
  ...
  async function remove(r: Registro) {
    ...
    const { error } = await supabase.from('psico_registros').delete().eq('id', r.id);
