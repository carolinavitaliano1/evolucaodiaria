...
export function ReunioesPanel({ patientId }: Props) {
  ...
  const load = useCallback(async () => {
    const { data, error } = await supabase
      .from('psicom_reunioes').select('*').eq('patient_id', patientId)
  ...
  async function remove(r: Reuniao) {
    ...
    const { error } = await supabase.from('psicom_reunioes').delete().eq('id', r.id);
  ...
  async function updateStatus(r: Reuniao, status: ReuniaoStatus) {
    const { error } = await supabase.from('psicom_reunioes').update({ status }).eq('id', r.id);
  ...
  async function save() {
    ...
    try {
      ...
      const { error } = existing
        ? await supabase.from('psicom_reunioes').update(payload).eq('id', existing.id)
        : await supabase.from('psicom_reunioes').insert(payload);
