...
export function TeamFinancialDashboard({ clinicId }: TeamFinancialDashboardProps) {
  ...
  const totalSessions = filteredEvolutions.filter(e => e.attendanceStatus === 'presente' || (e.attendanceStatus === 'reposicao' || e.attendanceStatus === 'anteposicao')).length;
  ...
  const memberStats = useMemo(() => {
    return members.map(member => {
      const memberEvos = monthlyEvolutions.filter(e => e.userId === member.userId);
      const sessions = memberEvos.filter(e => e.attendanceStatus === 'presente' || (e.attendanceStatus === 'reposicao' || e.attendanceStatus === 'anteposicao')).length;
  ...
  const patientBreakdown = useMemo(() => {
    return patientIdsInFilter
      .map(patientId => {
        ...
        const sessions = patientEvos.filter(e => e.attendanceStatus === 'presente' || (e.attendanceStatus === 'reposicao' || e.attendanceStatus === 'anteposicao')).length;
    ...
    <div className="space-y-5">
      ...
      {(() => {
        ...
        if (selMember?.remunerationType === 'fixo_mensal') {
          ...
          const days = new Set(filteredEvolutions.filter(e => e.attendanceStatus === 'presente' || (e.attendanceStatus === 'reposicao' || e.attendanceStatus === 'anteposicao')).map(e => e.date)).size;
                  ...
                  />
                   <p className="text-xs text-muted-foreground">
                    ...
                    {member.remunerationType === 'fixo_dia' && (() => {
                      const days = new Set(monthlyEvolutions.filter(e => e.userId === member.userId && (e.attendanceStatus === 'presente' || (e.attendanceStatus === 'reposicao' || e.attendanceStatus === 'anteposicao'))).map(e => e.date)).size;
