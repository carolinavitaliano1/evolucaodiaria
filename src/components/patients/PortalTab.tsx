...
import { Send, Loader2, Mail, RefreshCw, CheckCircle2, Clock, MessageSquare, FileText } from 'lucide-react';
...
export function PortalTab({ patientId, patientEmail, patientName }: PortalTabProps) {
  ...
  const loadData = async () => {
    ...
    const [{ data: account }, { data: msgs }, { data: form }] = await Promise.all([
      ...
      supabase.from('patient_intake_forms').select('submitted_at, full_name, phone, health_info').eq('patient_id', patientId).maybeSingle(),
    ]);
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h3 className="font-semibold text-sm text-foreground flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-xs">ED</span>
              Portal do Paciente
            </h3>
            {portalAccount ? (
      ...
      {portalAccount?.status === 'active' && (
        <div className="bg-card rounded-xl border border-border p-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="font-semibold text-sm text-foreground">Ficha de Cadastro</h3>
          </div>
          {intakeForm?.submitted_at ? (
            <div className="space-y-1">
              <p className="text-xs text-success font-medium flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3" />
                Enviada em {format(new Date(intakeForm.submitted_at), "d 'de' MMMM 'às' HH:mm", { locale: ptBR })}
              </p>
              {intakeForm.full_name && <p className="text-xs text-muted-foreground">Nome: {intakeForm.full_name}</p>}
              {intakeForm.phone && <p className="text-xs text-muted-foreground">Tel: {intakeForm.phone}</p>}
            </div>
          ) : (
            <p className="text-xs text-muted-foreground">Paciente ainda não preencheu a ficha.</p>
          )}
        </div>
      )}
        ...
        <div className="bg-card rounded-xl border border-border overflow-hidden">
          ...
          <div className="p-4 border-b border-border space-y-2">
            ...
            <Textarea
              ...
              placeholder={messageType === 'tarefa' ? `Tarefa para ${patientName}...` : messageType === 'feedback' ? `Feedback da sessão...` : `Escreva uma mensagem para ${patientName}...`}
