      ...
      <Tabs defaultValue={(typeof window !== 'undefined' && new URLSearchParams(window.location.search).get('tab')) || 'today'} className="space-y-4 lg:space-y-6">
        <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
          {[
            ...
            ...(clinic.type === 'clinica' ? [{ value: 'agenda-settings', icon: <CalendarOff className="w-5 h-5" />, label: 'Config. Agenda', color: 'text-rose-500' }] : []),
          ].map(tab => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {clinicPatients.filter(p => !patientSearch || p.name.toLowerCase().includes(patientSearch.toLowerCase())).map((patient) => (
                  ...
                  >
                    ...
                    {patient.paymentValue && clinic?.type === 'clinica' && clinic?.paymentType !== 'fixo_mensal' && clinic?.paymentType !== 'fixo_diario' && (() => {
